...
- [ ] T-12. (написано, ждёт VPS) `deploy/bootstrap.sh` (sudo, один раз): podman, git, пользователь `iiko_copywriter`, linger, ключ с ноутбука → `podman info` под `iiko_copywriter` работает (R-12)
...
- Админские команды загрузки данных для обучения и артефактов (список админов `MARKETEER_ADMIN_IDS` уже есть, пока за ним только `/update`).
